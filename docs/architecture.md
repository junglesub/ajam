...
- 새 업무 기록을 작성할 때 이전 업무일에 연결됐고 선택 날짜에도 진행중인 Notion 카드가 자동 추천된다. 새로고침 직후 기본 선택 날짜도 같은 추천 경로를 사용한다.
- 요일별 자동 Notion 카드는 이전 업무일 카드 추천보다 먼저 적용되며 `weekday_default` source로 저장된다. 이후 날짜의 이전 카드 추천은 이 source를 제외해 반복 회의 카드가 사람의 직접 선택처럼 전파되지 않게 한다.
- Notion 카드 후보는 업무 entry에 명시적으로 매핑될 때만 분석에 포함된다. 카드별 시간은 기본 자동 균등 배분이고, 사용자가 수동으로 바꾼 카드 시간은 보존한다. 수동 카드가 있으면 자동 카드는 남은 업무 시간을 균등 배분받고, 카드 배분 합계가 업무 시간과 달라도 업무 기록 저장은 허용한다.
...
- GitHub Actions는 n8n package version 변경과 n8n node 검증 성공이 함께 충족될 때 `packages/n8n-nodes-ajam`을 public npm registry에 `@junglesub/n8n-nodes-ajam`으로 publish한다. Publish 인증은 npm Trusted Publishing OIDC 연결을 사용한다.
...
image 검증 기준은 install, lint, typecheck, workspace seed, web build, Docker image build, Docker web server smoke test이다. seed 검증은 CI workspace에서 `pnpm db:seed`로 수행하고, Docker smoke test는 production image를 컨테이너로 실행한 뒤 `GET /api/health` 응답만 확인한다. production runner image에는 workspace와 pnpm 실행 환경이 없으므로 CI는 Docker image 안에서 workspace script를 실행하지 않는다. `better-sqlite3`는 native binding을 필요로 하므로 root `package.json`의 pnpm `onlyBuiltDependencies`에서 빌드를 승인한다. n8n node 검증 기준은 install, n8n node typecheck, n8n node build이다. `main` 브랜치 push에서는 image 관련 파일이 변경된 경우 GHCR image를 publish한 뒤 DCA deploy request를 보내고, n8n package version이 변경된 경우 n8n package를 배포한다.
