...
- 시간 입력 모드는 업무 프로젝트, 휴가, 공휴일을 카테고리로 보고 사용자가 카테고리 순서를 조정할 수 있게 한다.
- 매크로 실행은 각 카테고리에서 1일부터 말일까지 순회하며 평일 필드는 값 입력 또는 빈칸 Tab을 수행하고, 주말은 입력과 Tab 모두 생략한다.
- 카테고리 말일 이후에는 다음 카테고리 1일 위치로 이동하기 위해 추가 Tab 5회를 수행한다.
