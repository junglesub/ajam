...
- 휴가 유형 이름 변경은 별도 유형 엔터티를 만들지 않고 선택한 연도의 같은 정규화 이름을 가진 현재/레거시 휴가 저장 행을 한 트랜잭션으로 갱신한다.
- 유형 이름이 합쳐져도 휴가 시간, 상태, 날짜는 유지하며 기존 이름의 색상 설정은 새 이름으로 이전하지 않는다. 새 이름에 저장된 설정이 없으면 자동 색상을 사용한다.
