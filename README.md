...
aJam 자동화를 위한 n8n custom node package는 `packages/n8n-nodes-ajam`에 있으며 GitHub Packages에 `@junglesub/n8n-nodes-ajam`으로 publish됩니다. 현재 제공 액션은 업무 기록 미작성 대상 조회와 리마인더 발송 기록입니다.
...
mkdir -p ~/.n8n/custom
cd ~/.n8n/custom
pnpm add /path/to/ajam/dist/junglesub-n8n-nodes-ajam-0.1.0.tgz
```
